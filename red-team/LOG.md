...
### RT-2026-007 — Weak-record discrimination and calibration-power collapse
**Target:** empirical identifiability of the optional selector echo after the coherent-mismatch null is independently specified.  
**Attack:** Grant the selector law and the hardest local coherent-mismatch null, choose `epsilon=sqrt(eta)` so the two models agree at leading order, and ask whether their higher-order difference carries enough information to be measured before calibration/systematic uncertainty dominates.  
**Method:** analytic asymptotics + Bernoulli information/power calculation + Wolfram symbolic/numerical checks + pulse-inverse/coherent-calibration literature comparison.  
**Result:** weakened / design narrowed, not defeated.  
**Evidence:** For `nu_selector=cos(theta)^(N eta)` and `nu_null=cos(sqrt(eta) theta)^N`, the log echoes agree through the quadratic term. Wolfram gives `log nu_null-log nu_selector = N eta(1-eta) theta^4/12 + O(theta^6)`. For a binary X/parity readout with `p=(1+nu)/2`, the per-shot KL divergence satisfies `D_KL(selector||null)=N eta(1-eta)^2 theta^6/288+O(theta^8)`. Thus target-shot information collapses as `theta^6` in the weak-record regime. Under an optimistic fixed-model normal approximation (one-sided alpha=0.05, 80% power, no nuisance uncertainty), `M≈890.29/[N eta(1-eta)^2 theta^6]`. For `N=8, eta=0.05`, illustrative counts fall from about `5.7e5` shots at `theta=0.4` to about `2.4e2` at `theta=1.3`, showing that the useful statistical curvature lies mainly outside the weak-record regime. Separately, `d nu_null/d epsilon=-N theta tan(epsilon theta)nu_null`; requiring a 95% calibration half-width smaller than half the selector-null gap gives the weak-angle criterion `sigma_epsilon/epsilon < (1-eta)theta^2/47.04`, demanding sub-percent relative calibration at small `theta`. Pulse-inverse/KIK literature provides a physically grounded forward/inverse construction but also exposes structured higher-order residuals; CAFE/error-amplification work provides a possible independent coherent-error calibration channel; Fisher-information work shows that a single fixed readout can be locally blind to coherent faults. Full derivations and caveats are in `experiments/ECHO_POWER_CALIBRATION_2026-08-24.md`.  
**Required change:** Do not design the selector test around a near-zero-angle power-law fit or assume that more weak-record points solve identifiability. Use a separate, information-complete nuisance-calibration channel; freeze the ordinary-noise prediction with propagated uncertainty; then test held-out interaction strengths with enough curvature to discriminate while explicitly constraining leakage, drift, nonlinear control and non-Markovian effects. Optimize statistical information jointly with nuisance/systematic risk rather than maximizing `theta` or redundancy alone.  
**Next attack:** Build a platform-specific pulse-level nuisance model and a predeclared calibration/holdout design; include leakage/non-Markovian uncertainty and independent `eta` calibration, then optimize held-out `(theta,N)` points by expected likelihood/KL separation subject to those constraints.

### RT-2026-008 — Can coherent mismatch be calibrated without creating the record?
**Target:** the leading coherent inverse-amplitude nuisance in P1 / RT-2026-006 and the concern in RT-2026-007 that calibration uncertainty may dominate the selector-null gap.  
**Attack:** Search for a native-pulse-matched control that suppresses the hypothesized record/selector mechanism while retaining high information about the coherent mismatch.  
**Method:** analytic controlled-rotation construction + Wolfram symbolic matrix check + Fisher-information calculation + comparison with superconducting robust-phase-estimation and cycle-calibration literature.  
**Result:** promising partial resolution for the coherent-mismatch direction; unresolved for the full hardware null.  
**Evidence:** For the symmetric gate `U(theta)=|0><0|⊗R_x(theta)+|1><1|⊗R_x(-theta)`, an environment fragment prepared in `|0_z>` is record-forming with branch overlap `cos(theta)`. Preparing the same fragment in the common generator eigenstate `|+_x>` uses the same conditional interaction but yields branch states that differ only by phase, so the designated overlap has unit modulus and `Sigma=0`. If the attempted inverse leaves residual fraction `epsilon`, Wolfram gives record-forming residual overlap `cos(epsilon theta)` but no-record residual overlap `exp(i epsilon theta)`. For `N` fragments the latter becomes a system Ramsey phase `N epsilon theta`; a suitable binary quadrature has exact per-shot Fisher information `N^2 theta^2`. Combining this with the prior weak-angle calibration criterion gives an idealized calibration requirement `M_cal > 2212.7616/[eta(1-eta)^2 N^2 theta^6]`, versus the prior optimistic target requirement `M_target≈890.29/[N eta(1-eta)^2 theta^6]`, hence `M_cal/M_target≈2.48544/N`. At `N=8, eta=0.05, theta=1`, the illustrative values are roughly 766 calibration shots versus 2466 target shots. Rudinger et al. (2025) independently demonstrate Heisenberg-limited coherent entangling-gate calibration with robust phase estimation on superconducting hardware, supporting the experimental plausibility of phase-amplified calibration as a primitive. Full derivation and limitations are in `experiments/NO_RECORD_RAMSEY_CONTROL_2026-08-31.md`.  
**Required change:** Treat coherent inverse mismatch as having a concrete candidate independent calibration channel rather than as an undifferentiated blocker. Any proposed implementation must deliberately choose a conditional-gate family with a common eigenstate, verify that the compiled pulse/context noise remains comparable between record-forming and no-record preparations, use both Ramsey quadratures/robust phase estimation, and separately calibrate stochastic errors, leakage, crosstalk, hidden records, drift and non-Markovian effects. Do not infer the full null from this one parameter.  
**Next attack:** Simulate or experimentally characterize the symmetric conditional gate after CZ+single-qubit compilation with realistic leakage/crosstalk and state-dependent noise. Test whether the no-record control remains predictive for the coherent part of the record-forming inverse error and whether fragment correlations spoil the `N^2` information scaling.
