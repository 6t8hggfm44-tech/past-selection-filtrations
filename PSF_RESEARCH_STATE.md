...
7. **Constructive extension.** Separate theorem, conjecture, optional postulate, and interpretation; test each additional axiom for consistency and empirical content. The 2026-09-07 compiled-path audit adds a concrete completion question: specify whether/how transient record distinguishability contributes to the selector's accumulated quantity `Σ`, rather than leaving path dependence implicit in endpoint notation.
...
- Does a nominally “no-record” logical calibration arm create transient branch-distinguishing records during compilation or in leaked/spectator/control degrees, and if so does the optional selector respond to those transient records or only to a designated endpoint?
...
- A mechanism-null control must be null on the mechanism-relevant **physical trajectory**, not merely at the logical endpoint, whenever the proposed mechanism may accumulate or respond to transient intermediate states. If endpoint-only behavior is assumed, record that as an explicit optional physical postulate rather than smuggling it in through an effective-gate description.
