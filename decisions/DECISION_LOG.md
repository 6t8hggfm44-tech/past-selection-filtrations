...
This file records changes that affect claims, priorities, interpretation, or publication language.

## D-2026-001 — July 2026 formulation is the baseline
**Date recorded:** 2026-08-07  
**Decision:** Treat the July 2026 revision as the current conceptual baseline rather than the November 2025 formulation.  
**Reason:** The revision more clearly separates the base record-filtration framework from optional additional physical postulates and narrows claims about decoherence, AQFT expectations, and empirical content.  
**Consequence:** Agent comparisons must not silently import stronger claims from superseded versions.

## D-2026-002 — Empirical discriminator is highest-priority scientific target
**Date recorded:** 2026-08-07  
**Decision:** Prioritize deriving or ruling out a distinctive empirical consequence of any optional PSF physical extension.  
...
## D-2026-006 — Weak-record selector/noise discrimination is information-starved
**Date:** 2026-08-24  
**Previous position:** After D-2026-005, the echo protocol required an independently calibrated ordinary inverse-error model and a sufficiently broad held-out interaction-strength sweep, but the statistical cost of remaining in the weak-record regime had not been quantified.  
**New position:** Do not use a near-zero-angle power-law fit as the principal selector discriminator. For the adversarial locally matched coherent-mismatch null `epsilon=sqrt(eta)`, `nu_null=cos(sqrt(eta)theta)^N` and `nu_selector=cos(theta)^(N eta)` agree through quadratic order. Their log separation begins at `N eta(1-eta)theta^4/12`, while a binary X/parity measurement has per-shot KL divergence `N eta(1-eta)^2 theta^6/288+O(theta^8)`. Under an optimistic fixed-model normal approximation (one-sided alpha=0.05, 80% power, no nuisance uncertainty), the required target-shot count scales as `M≈890.29/[N eta(1-eta)^2 theta^6]`. A separate weak-angle calibration criterion gives `sigma_epsilon/epsilon < (1-eta)theta^2/47.04` if the 95% coherent-mismatch uncertainty is required to stay below half the selector-null gap. These scalings make the weak-record regime intrinsically inefficient for this particular null. The preferred next design is a platform-specific independent nuisance-calibration stage followed by frozen standard-noise predictions and held-out interaction strengths chosen for discriminating information subject to leakage, drift, nonlinear-control and non-Markovian constraints.  
**Evidence/reason:** Wolfram symbolic series, Bernoulli KL and normal-power calculations recorded in `experiments/ECHO_POWER_CALIBRATION_2026-08-24.md`; Henao–Santos–Uzdin (2023) and Bar–Santos–Uzdin (2026) on operational pulse inverses/KIK and structured residual terms; Debroy et al. (2023) on context-aware coherent/incoherent calibration; Pan et al. (2026) on coherent-fault Fisher-information blindness of insufficient readout designs; Zhang et al. (2026) as platform-specific evidence that high-precision coherent calibration is feasible but not automatically sufficient for a full echo-null model.  
**Affected claims/files:** `PSF_RESEARCH_STATE.md` P1/current priority; `open-problems/QUEUE.md` P1; `red-team/LOG.md` RT-2026-007; `experiments/ECHO_POWER_CALIBRATION_2026-08-24.md`; `literature/LEDGER.md`.  
**Consequence:** P1 remains alive but narrower. Statistical separation and nuisance identifiability must be designed together; more weak-record samples or more fragments alone are not adequate evidence.  
**Reversible if:** a complete selector dynamics yields a discriminator that differs from standard dynamics at lower order or in an orthogonal observable; a proposed implementation proves the matched coherent-mismatch channel irrelevant below sensitivity; or a different readout/control design yields substantially better nuisance-robust information scaling.

## D-2026-007 — Logical no-record endpoints are insufficient; require trajectory-level mechanism audit
**Date:** 2026-09-07  
**Previous position:** The 2026-08-31 common-eigenstate Ramsey construction supplied a promising candidate mechanism-null calibration arm at the level of an effective symmetric conditional gate: environment input `|+_x>` gives final designated branch-overlap magnitude one while coherent inverse mismatch appears as a Ramsey phase. The principal stated concern was whether compiled physical noise remained comparable between the record-forming and no-record preparations.  
**New position:** Do not infer selector/mechanism nulling from the effective gate endpoint alone. A standard exact digital decomposition `exp[-i theta Z⊗X/2]=(I⊗H) CNOT (I⊗R_z(theta)) CNOT (I⊗H)` makes the nominal no-record arm transiently **maximally record-forming**: after the first CNOT, the two environment branch states are `|0_z>` and `|1_z>` and have zero overlap, even though the second CNOT uncomputes them and the final overlap returns to unit modulus. A credible calibration arm therefore requires a trajectory-level audit under the predeclared physical filtration. If the optional selector is path/accumulation sensitive, the digital arm is not null; if the selector is endpoint-only, that path-independence is an additional physical rule that must be explicitly specified and justified by a complete selector dynamics.  
**Evidence/reason:** Exact analytic branch propagation and an independent 4x4 numerical matrix/overlap check recorded in `experiments/COMPILED_PATH_RECORD_AUDIT_2026-09-07.md`; the gate decomposition was verified to floating-point roundoff. Wolfram was attempted but unavailable during the run, so no Wolfram corroboration is claimed. Historical cross-resonance work shows that direct effective `ZX` Hamiltonians are experimentally realizable, providing a possible route to keep the common-eigenstate arm overlap-one throughout an ideal continuous trajectory; recent superconducting leakage/crosstalk control work supports pulse/Hamiltonian-level nuisance engineering but does not establish a PSF null.  
**Affected claims/files:** `PSF_RESEARCH_STATE.md` P1/current priority and P7 linkage; `open-problems/QUEUE.md` P1/P7; `red-team/LOG.md` RT-2026-009; `experiments/COMPILED_PATH_RECORD_AUDIT_2026-09-07.md`; `research-agent/TRANSFER_CANDIDATES.md`; relevant literature ledger entries.  
